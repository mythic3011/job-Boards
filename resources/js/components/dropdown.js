...
function initDropdowns() {
    ...
    $("[data-dropdown]").each((i, el) => {
        ...
        if ($arrow.length) {
            $arrow.addClass("transition-transform duration-200 rotate-0");
        }
        ...
        if (!$menu.attr("id"))
            $menu.attr(
                "id",
                "dropdown-menu-" + Math.random().toString(36).substr(2, 9),
            );
            ...
            switch (e.key) {
                ...
                case "ArrowDown":
                    if (current >= 0) {
                        e.preventDefault();
                        items
                            .eq(current < items.length - 1 ? current + 1 : 0)
                            .focus();
                    }
                    break;
                case "ArrowUp":
                    if (current >= 0) {
                        e.preventDefault();
                        items
                            .eq(current > 0 ? current - 1 : items.length - 1)
                            .focus();
                    }
                    break;
                case "Tab":
