...
<div class="max-w-4xl mx-auto">
    @if(isset($job))
        ...
        <nav class="theme-text-muted mb-4 flex items-center gap-2 text-sm">
            ...
            <span class="theme-text-strong font-medium">{{ $job->title }}</span>
        </nav>

        <x-ui.card padding="p-8">
            {{-- Header section --}}
            <div class="theme-table-divider mb-6 border-b pb-6">
                <h1 class="theme-text-strong mb-3 text-3xl font-bold">{{ $job->title }}</h1>

                <div class="theme-text-muted flex flex-wrap items-center gap-4 text-sm">
                    {{-- Company info --}}
                    <div class="flex items-center gap-2">
                        <div class="theme-icon-tile-accent flex h-8 w-8 items-center justify-center rounded-md font-semibold text-xs uppercase">
                            {{ substr($job->companyUser->nickname ?? '?', 0, 2) }}
                        </div>
                        <span class="theme-text-strong font-medium">{{ $job->companyUser->nickname }}</span>
                    </div>
            ...
            {{-- Job details --}}
            <div class="space-y-6">
                <div class="theme-panel-subtle rounded-2xl border p-5 sm:p-6">
                    <h2 class="theme-text-strong mb-3 flex items-center gap-2 text-xl font-semibold">
                        <svg style="width:20px;height:20px" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="theme-link">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        Requirements
                    </h2>
                    <div class="theme-text-muted prose max-w-none pl-7 leading-relaxed">
            ...
            {{-- Action buttons --}}
            <div class="theme-table-divider mt-8 border-t pt-6">
