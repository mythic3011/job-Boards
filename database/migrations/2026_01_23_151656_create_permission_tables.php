...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['roles'], static function (Blueprint $table) use ($teams, $columnNames) {
            ...
            // unique: team foreign key, name, guard name
            if ($teams || config('permission.testing')) {
        ...
        Schema::create($tableNames['model_has_roles'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole, $teams) {
            ...
            // FK: role id from roles table
            $table->foreign($pivotRole)
                ->references('id')
                ->on($tableNames['roles'])
                ->onDelete('cascade');

            // unique: team foreign key, role id, model morph key, model type
            if ($teams) {
                $table->unsignedBigInteger($columnNames['team_foreign_key']);
                $table->index($columnNames['team_foreign_key'], 
    ...
    public function down(): void
    {
        ...
        throw_if(empty($tableNames), Exception::class, 
